...
# It needs 3 arguments as input. First argument is input directory which contain flac files.
...
do
	...
	then
       		sox --norm="$normValue" "${dir%/}" "$outdir"/"${subDir%.flac}"-Norm0.flac
      	fi
	...
	then
		mkdir "$outdir"/"$subDir"
		for track in "$dir"/*.flac
		do
			file=$(basename "$track")
			sox --norm="$3" "$track" "$outdir"/"$subDir"/"${file%.flac}"-Norm0.flac
		done
	fi
done
